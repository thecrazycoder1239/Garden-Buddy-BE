  ...
  if (currentInd < arr.length - 1) {
    return arr[currentInd]
      .then((result) => {
        return Promise.all([
          result,
          returnOrderedPromiseChain(arr, currentInd + 1),
        ]);
      })
    ...
    return arr[currentInd].then((result) => {
      return [result];
    });
  }
};

//Note this file has been tested using insomnia. Should you edit, it please use insomnia to test it.

const handleComposition = (req, res, next) => {
  const { compositeRequest } = req.body;
  if (!compositeRequest) {
    next({
      msg: "Missing compositeRequest field, requires key of compositeRequest",
    });
    ...
    next({
      msg: "compositeRequest requests must contain ref, method and path",
    });
  } else {
    ...
    returnOrderedPromiseChain(promiseArray, 0).then((result) => {
